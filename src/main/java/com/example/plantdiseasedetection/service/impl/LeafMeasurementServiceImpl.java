...
import com.example.plantdiseasedetection.payload.VeinFeatures;
...
//            System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
    ...
    private static double getConvexHullArea(BufferedImage image) {
        ...
        List<Point> leafPoints = findLeafPoints(image);
        List<Point> convexHullPoints = findConvexHull(leafPoints);
        ...
        for (int i = 0; i < n; i++) {
            ...
            Point p2 = convexHullPoints.get((i + 1) % n);
    ...
    private static List<Point> findLeafPoints(BufferedImage image) {
